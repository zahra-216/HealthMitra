...
import { VitalSigns } from "@/types"; // Keep the import for prop type

// =================================================================
// Zod Schema and Form Data Type
// =================================================================
...
type FormData = z.infer<typeof vitalSignsSchema>;
...
interface VitalSignsFormProps {
  onSubmit: (data: VitalSigns) => void; // Prop still uses the original VitalSigns type
...
const VitalSignsForm: React.FC<VitalSignsFormProps> = ({
  ...
  const {
    ...
    // *** FIX 1: Use FormData (inferred from Zod) as the generic type ***
    resolver: zodResolver(vitalSignsSchema), // Cast initialData to the correct FormData type if it exists
    defaultValues: initialData as FormData,
  }); // The watch values are now correctly typed as number | undefined

  const weight = watch("weight");
  const height = watch("height"); // *** FIX 2: Explicitly assert as number in arithmetic operations ***
  const bmi =
    weight !== undefined && height !== undefined && height > 0
      ? ((weight as number) / Math.pow((height as number) / 100, 2)).toFixed(1)
      : null; // *** FIX 3: Create a handler to cast FormData back to VitalSigns (optional, but safer) *** // Since the schema is a superset of the type, we can cast it.

  const handleFormSubmit: SubmitHandler<FormData> = (data) => {
    // Cast the validated data back to the expected VitalSigns type for the prop
